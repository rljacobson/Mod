...
pub use super::{VariableDagNode, VariableLHSAutomaton, VariableSymbol};
use crate::{
  abstractions::{hash2 as term_hash, rc_cell, IString, NatSet, RcCell},
  core::{
    automata::RHSBuilder,
  ...
  theory::{DagNode, NodeCache, RcDagNode, RcLHSAutomaton, RcSymbol, RcTerm, Term, TermMembers},
};
...
impl Term for VariableTerm {
  ...
  fn compile_lhs(
    ...
    let mut automaton: RcLHSAutomaton =
      rc_cell!(VariableLHSAutomaton::new(self.index, self.sort().clone(), match_at_top));
...
impl Formattable for VariableTerm {
  fn repr(&self, style: FormatStyle) -> String {
    match style {
      FormatStyle::Simple => self.name.to_string(),
