...
use crate::{
  core::{
    ...
    pre_equation::{PreEquation, RcPreEquation},
  },
  theory::{
...
impl FreeRemainder {
  ...
  pub fn new(
    ...
    // runtime treatment.
    let mut fast: Speed = if !(equation.borrow().has_condition()) {
      Speed::SuperFast
    } else {
      Speed::Slow
    };
    ...
    for oc in free_symbols {
      ...
      if free_term.term_members.save_index != NONE {
        ...
        // equation.borrow().repr(FormatStyle::Simple)).as_str()
        assert!(parent.slot_index != NONE, "bad slot for parent in equation");
        let new_free_var = FreeVariable {
          position:  slot_translation[parent.slot_index as usize],
          arg_index: oc.arg_index,
          var_index: index,
          sort:      free_term.connected_component().borrow().sort(0).upgrade().unwrap(),
        };
