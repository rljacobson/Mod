...
struct FreeFast3RHSInstruction {
  pub(crate) symbol:      RcSymbol,
  pub(crate) destination: i32,
  pub(crate) sources:     [i32; 3],
}


#[derive(Default)]
pub struct FreeFast3RHSAutomaton {
  symbol:            Option<RcSymbol>,
  instructions:      Vec<FreeRHSAutomatonInstruction>,
  fast_instructions: Vec<FreeFast3RHSInstruction>,
  sources:           [i32; 3],
  destination:       i32,
}

impl FreeFast3RHSAutomaton {
  #[inline(always)]
  fn fill_out_args(&self, instruction: &FreeFast3RHSInstruction, matcher: &Substitution, dag_node: &mut dyn DagNode) {
    dag_node
    ...
    dag_node
      .dag_node_members_mut()
      .args
      .push(matcher.value(instruction.sources[2] as usize).unwrap());
  }
}

impl RHSAutomaton for FreeFast3RHSAutomaton {
  ...
  fn remap_indices(&mut self, variable_info: &mut VariableInfo) {
    ...
    for i in 0..instruction_count {
      ...
      let mut f = FreeFast3RHSInstruction {
        symbol: instr.symbol.clone(),

        destination: instr.destination,
        sources:     [0, 0, 0],
      };
      // `instr.sources` may have fewer than 3 elmts.
  ...
  fn construct(&self, matcher: &mut Substitution) -> MaybeDagNode {
    ...
    let mut instruction = if let Some(i) = instruction_iter.next() {
      i
    } else {
      return None;
    };
