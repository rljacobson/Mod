...
use crate::core::rewrite_context::{make_subcontext, Purpose, RcRewritingContext};
use crate::theory::{find_available_terms, index_variables};
...
impl ConditionFragment {
  pub fn check(&mut self, variable_info: &mut VariableInfo, bound_variables: &mut NatSet) {
    ...
    //   "{:?}: all the variables in the left-hand side of assignment condition fragment {:?} are bound before the matching takes place.",
    //   self.lhs,
    //   self
    // );
  ...
  pub fn preprocess(&mut self) {
    match self {
      Assignment {lhs_term, rhs_term, .. } => {
        ...
        assert!(lhs_term.borrow().connected_component() == rhs_term.borrow().connected_component(), "component clash");
      ...
      SortTest {lhs_term, sort, .. } => {
        lhs_term.borrow_mut().fill_in_sort_info();
        assert!(lhs_term.borrow().connected_component() == sort.borrow().sort_component, "component clash");
      }
    }
  }
  ...
  pub fn compile_build(&mut self, variable_info: &mut VariableInfo, available_terms: &mut TermBag) {
    match self {
      Equality {lhs_term, rhs_term, lhs_index, rhs_index,  builder, .. } => {
      ...
      SortTest {lhs_term, lhs_index, builder, .. } => {
        *lhs_index = compile_rhs(lhs_term.clone(), builder, variable_info, available_terms, true);
        variable_info.use_index(*lhs_index);
        variable_info.end_of_fragment();
      }

      Assignment {lhs_term, rhs_term, rhs_index, builder, .. } => {
        *rhs_index = compile_rhs(rhs_term.clone(), builder, variable_info, available_terms, true);
        variable_info.use_index(*rhs_index);
      ...
      Rewrite {lhs_term, rhs_term, lhs_index, builder, .. } => {
  ...
  pub fn compile_match(&mut self, variable_info: &mut VariableInfo, bound_uniquely: &mut NatSet) {
    match self {
      ...
      Equality { lhs_index, rhs_index, builder, .. } => {
      ...
      Assignment { lhs_term, rhs_index, lhs_matcher, builder, .. } => {
        builder.remap_indices(variable_info);
        *rhs_index = variable_info.remap_index(*rhs_index);
      ...
      Rewrite { rhs_term, lhs_index, rhs_matcher, builder, .. } => {
        builder.remap_indices(variable_info);
        *lhs_index = variable_info.remap_index(*lhs_index);
  ...
  ) -> bool
  {
    match self {
      ...
      Equality { builder, lhs_index, rhs_index, .. } => {
        if !find_first { return false; }
